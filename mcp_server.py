...
@mcp.tool()
def search_graph(keywords: str) -> str:
    """
    在知识图谱中搜索冲突对和联想关系。

    输入逗号分隔的关键词，返回图谱中找到的高幽默价值冲突关系。
    图谱包含23万节点，来源于"妈的欧洲账本"、脱口秀、YouTube等。

    Args:
        keywords: 逗号分隔的关键词，如"堵车,租房,咖啡,通勤"
    """
    global _graph
    ...
    try:
        ...
        from src.knowledge.graph import find_topic_node, find_humor_slots

        results = []
        for kw in kw_list:
            node = find_topic_node(_graph, kw)
            if not node:
                results.append(f"❌ {kw}: 不在图谱中")
                continue

            slots = find_humor_slots(_graph, kw, top_k=5)
            if slots:
                results.append(f"\n✅ {kw} (节点: {node})")
                for s in slots:
                    results.append(f"  → [{s['relation']}] → {s['slot']} (score={s['score']:.1f})")
                    if s.get("path"):
                        results.append(f"    路径: {' → '.join(str(p) for p in s['path'])}")
            else:
                results.append(f"⚠️ {kw}: 在图谱中但没有高价值冲突关系")

        return f"图谱: {_graph.number_of_nodes()} 节点, {_graph.number_of_edges()} 边\n\n" + "\n".join(results)

    except Exception as e:
        return f"图谱加载失败: {e}\n\n这不影响写作，Claude 自己联想即可。"
...
def match_images(post_text: str) -> str:
    ...
    使用前需要先构建图片索引（运行 pipeline/images/photo_index.py）。
...
@mcp.tool()
def get_humor_techniques(mechanism: str = "", top_k: int = 10) -> str:
    """
    从脱口秀分析中提取笑点技巧，供文案创作参考。

    只提取"怎么搞笑"的技巧（机制、冲突对、可复用规则），
    不提取脱口秀的风格/语气——最终风格始终以"妈的欧洲账本"为准。

    Args:
        mechanism: 筛选特定笑点机制（如 expectation_violation, deflation），空=全部
        top_k: 返回几条
    """
    # 尝试加载脱口秀分析
    standup_cache = os.path.join(LOCAL_CACHE_DIR, "analyzed_standup.json")
    standup = []

    if os.path.exists(standup_cache):
        with open(standup_cache, "r", encoding="utf-8") as f:
            standup = json.load(f)
    else:
        # 从 GCS 加载
        try:
            blobs = list(get_bucket().list_blobs(prefix="data/analyzed_standup/"))
            for b in blobs:
                if b.name.endswith(".json"):
                    try:
                        standup.append(json.loads(b.download_as_text(encoding="utf-8")))
                    except Exception:
                        pass
            if standup:
                os.makedirs(LOCAL_CACHE_DIR, exist_ok=True)
                with open(standup_cache, "w", encoding="utf-8") as f:
                    json.dump(standup, f, ensure_ascii=False)
        except Exception:
            pass

    if not standup:
        return "脱口秀分析库为空。请先运行：python pipeline/analyze/analyze_posts.py --source 脱口秀"
    ...
    for post in standup:
        ...
        for p in post.get("punchlines", []):
            s = p.get("structure", {})
            m = s.get("mechanism", "")
            if mechanism and mechanism != m:
                continue
            techniques.append({
                "source_title": title,
                "mechanism": m,
                "setup": s.get("setup_text", ""),
                "punchline": s.get("punchline_text", ""),
                "conflict_pair": s.get("conflict_pair", []),
                "pain_point": p.get("emotion", {}).get("pain_point", ""),
                "technique": p.get("language", {}).get("key_technique", ""),
                "micro_rules": p.get("craft", {}).get("micro_rules", []),
                "transferable": p.get("craft", {}).get("transferable", ""),
            })

    if not techniques:
        return f"没有找到{f'机制为{mechanism}的' if mechanism else ''}笑点技巧"

    # 格式化输出
    lines = [f"共 {len(techniques)} 条笑点技巧（来源: 脱口秀，仅供技巧参考，风格以妈的欧洲账本为准）\n"]
    for t in techniques[:top_k]:
        lines.append(f"### [{t['mechanism']}] {t['source_title']}")
        lines.append(f"- setup: {t['setup'][:80]}")
        lines.append(f"- punchline: {t['punchline'][:80]}")
        lines.append(f"- 冲突对: {t['conflict_pair']}")
        lines.append(f"- 痛点: {t['pain_point'][:60]}")
        if t['micro_rules']:
            lines.append(f"- 可复用规则: {t['micro_rules']}")
        if t['transferable']:
            lines.append(f"- 如何用在图文帖: {t['transferable'][:80]}")
        lines.append("")
...
@mcp.tool()
def feed_material(content: str, material_type: str = "other", source: str = "", tags: str = "") -> str:
    """
    添加素材到 feed 库。

    随时把小红书看到的好话题、脱口秀段子、生活灵感灌进系统。

    Args:
        content: 素材内容
        material_type: 类型 (topic/joke/life/reference/phrase/other)
        source: 来源（如"小红书""脱口秀大会"）
        tags: 标签，逗号分隔（如"租房,吐槽,LA"）
    """
    try:
        from pipeline.feed.feed_material import add_material, format_material
        tag_list = [t.strip() for t in tags.split(",") if t.strip()] if tags else []
        entry = add_material(content, material_type, source, tag_list)
        return f"已添加素材:\n{format_material(entry)}"
    except Exception as e:
        return f"添加失败: {e}"


@mcp.tool()
def analyze_xhs_link(url: str) -> str:
    """
    输入小红书链接，自动抓取内容 + 分析笑点 + 存入素材库。
    ...
    分析结果自动存入素材库，下次生成文案时可以用。

    Args:
        url: 小红书帖子链接
    """
    try:
        from pipeline.feed.fetch_xhs import fetch_and_analyze
        return fetch_and_analyze(url)
    except Exception as e:
        return f"分析失败: {e}"
...
def render_and_preview(post_text: str, title: str = "") -> str:
    """
    渲染帖子所有图片并打开预览文件夹 + 上传到 Google Drive（方便手机下载发布）。

    输入确认后的帖子文案（===图1=== 格式），系统会：
    1. 把每张图渲染成"妈的欧洲账本"风格图片
    2. 放到预览文件夹并打开 Finder
    3. 上传到 Google Drive 的 "小红书发布" 文件夹（手机可直接下载）
