...
def evaluate(topic, candidates, verbose=True):
    ...
    return sorted(scored, key=lambda x: x["scores"].get("total", 0), reverse=True) + unscored
...
def refine(topic, joke, scores, max_rounds=2):
    ...
    for _ in range(max_rounds):
        ...
        refined = call_gemini(REFINE_PROMPT.format(
            topic=topic, joke=current_joke, comment=comment,
...
def run(topic):
    ...
    if not scored:
        ...
        return ranked[0]
    best = scored[0]
    print(f"初始最优（[{best['method']}]，总分={best['scores'].get('total')}）：")
    print(f"\n  {best['joke']}")
