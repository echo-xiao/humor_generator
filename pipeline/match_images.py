...
from pipeline.photo_index import load_index, search_photos, get_drive_creds
...
重要：每张图都必须选一张照片，不允许留空！
如果没有完美匹配的照片，就选最接近的、氛围最搭的那张。
城市街景、日常场景、甚至抽象的照片都可以用来配文字型内容。
    ...
    for s in slides:
        ...
        if pick and pick.get("photo_id"):
            ...
            if photo_info:
                ...
                result["reason"] = pick.get("reason", "photo_id不在索引中")

        # Fallback: Gemini没选或选了null，用搜索候选第一名
        if not result["recommended"] and cands:
            fallback = cands[0]
            result["recommended"] = {
                "id": fallback["id"],
                **{k: v for k, v in fallback.items() if k != "id"},
            }
            reason = pick.get("reason", "") if pick else ""
            result["reason"] = f"[次相关fallback] {reason}".strip()
